      ...
      <div class="key-settings" id="key-settings">
        <div class="key-settings-row">
          <label class="field" id="script-type-field">Script type
            <select id="script-type"><option value="bip44">Legacy · BIP44</option><option value="bip49">Nested SegWit · BIP49</option><option value="bip84" selected>Native SegWit · BIP84</option><option value="bip86">Taproot · BIP86</option></select>
          </label>
        ...
        <div class="key-settings-row" id="account-address-settings">
          <label class="field">Account
            ...
            <span class="field-note" id="account-help">Hardened account index · 0 to 2,147,483,647</span>
          </label>
...
function hodlBuildMultisigCosignerExports(root,network,accountIndex,masterFingerprint){
  ...
  return[
    ...
    return{...definition,accountPath,originPath,prefix,value:`[${masterFingerprint}/${originPath}]${publicKey}`}
  })
}
...
Hs=function(root,network,count,source,accountIndex=0){
  ...
  return{
    ...
    rootPrivateLabel:cr[network].x.prvName,rootPublicLabel:cr[network].x.pubName,masterFingerprint,
    multisigCosignerExports:root.privateKey?hodlBuildMultisigCosignerExports(root,network,accountIndex,masterFingerprint):[],
    imported:!1,notes:source.notes,warnings:source.warnings,accounts
  }
};
...
var hodlBip39WordSet=new Set(Ae),hodlBip39WordIndex=new Map(Ae.map((word,index)=>[word,index])),hodlLastWordCache=new Map();
var hodlOnScreenKeyboardOpen=!1;
...
function hodlParseCards(raw,targetWords=Pt){
  ...
  let tokens=text.split(/[\s,.;:_|/-]+/).filter(Boolean),cards=[],invalid=[],duplicates=[];
  for(let token of tokens){
    let card=hodlNormalizeCardToken(token);
    if(!card){invalid.push(token);continue}
    let pool=cards.length<needed.first?cards:cards.slice(needed.first);
    if(pool.includes(card))duplicates.push(card);
  ...
  return{cards,invalid,duplicates,bits,needed,hashInput:cards.join(" ")}
}
...
function hodlCardsEntropy(value,targetWords=Pt){
  ...
  if(parsed.invalid.length)return{ok:!1,error:`Cards use rank then suit, like AS, 10H, or TD. Ignored: ${parsed.invalid.slice(0,8).join(" ")}`,notes,warnings,parsed};
  if(parsed.duplicates.length)return{ok:!1,error:`Do not repeat a card in the same shuffle. Repeated: ${parsed.duplicates[0]}.`,notes,warnings,parsed};
  if(!parsed.cards.length)return{ok:!1,error:"Deal at least one card from a shuffled deck.",notes,warnings,parsed};
  ...
  if(parsed.bits<config.bits)return{ok:!1,error:config.words===24?`A 24-word seed needs 256 bits. Deal all 52 unique cards, shuffle again, then deal 6 more (${parsed.cards.length} of ${required}).`:`A ${config.words}-word seed needs ${config.bits} bits. Deal ${parsed.needed.first} unique cards without putting them back (${parsed.cards.length} of ${parsed.needed.first}).`,notes,warnings,parsed};
  ...
  return{ok:!0,bytes,hex:M.encode(bytes),bits:config.bits,sourceBits:parsed.bits,method:"cards-sha256",notes,warnings,parsed}
}
function hodlUpdateCards(){
  let input=document.getElementById("cards");if(!input)return;
  let config=hodlSeedConfig(),parsed=hodlParseCards(input.value,config.words),required=parsed.needed.first+parsed.needed.extra,entropy=hodlCardsEntropy(input.value,config.words);
  ...
  try{if(parsed.cards.length)preview=_n(Z(new TextEncoder().encode(parsed.hashInput)).slice(0,config.bytes)).split(" ")}catch{}
  hodlRenderDiceWordGrid(wordsBox,preview,config.words,!entropy.ok);
  let meta=W("#cards-meta"),missing=Math.max(0,required-parsed.cards.length),status="";
  if(parsed.invalid.length)status=`${parsed.invalid.length} invalid token${parsed.invalid.length===1?"":"s"} \u00b7 use AS, 10H, or TD`;
  else if(parsed.duplicates.length)status=`Repeated ${parsed.duplicates[0]} in this shuffle \u00b7 deal a different card`;
  else if(!parsed.cards.length)status=`0 of ${required} cards \u00b7 0.0 bits \u00b7 ${config.words}-word seed`;
  else if(config.words===24&&parsed.cards.length>=52&&parsed.cards.length<required)status=parsed.cards.length===52?`52 of 52 first shuffle \u00b7 ${parsed.bits.toFixed(1)} bits \u00b7 shuffle again, then deal 6 more`:`Second shuffle \u00b7 ${parsed.cards.length-52} of 6 \u00b7 ${parsed.bits.toFixed(1)} bits \u00b7 ${required-parsed.cards.length} remaining`;
  else if(missing)status=`${parsed.cards.length} of ${required} unique cards \u00b7 ${parsed.bits.toFixed(1)} bits \u00b7 ${missing} remaining`;
  else status=`${parsed.cards.length} card${parsed.cards.length===1?"":"s"} \u00b7 ${parsed.bits.toFixed(1)} bits \u00b7 ready to derive`;
  meta.textContent=status;meta.className="muted"+(entropy.ok?" ok":parsed.invalid.length||parsed.duplicates.length?" err":"");
  document.querySelectorAll("[data-card-suit]").forEach(button=>{let active=button.getAttribute("data-card-suit")===hodlCardSuit;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))});
  let used=new Set(parsed.cards.length<parsed.needed.first?parsed.cards:parsed.cards.slice(parsed.needed.first));
  document.querySelectorAll("[data-card-rank]").forEach(button=>{let card=button.getAttribute("data-card-rank")+hodlCardSuit;button.disabled=used.has(card)});
  let undo=document.getElementById("card-undo");if(undo)undo.disabled=!parsed.cards.length&&!String(input.value||"").trim();
  hodlQueueMasterFingerprintPreview()
}
function hodlAppendCard(rank){
  let input=document.getElementById("cards");if(!input)return;
  let card=hodlNormalizeCardToken(rank+hodlCardSuit);if(!card)return;
...
function hodlComputeTargetLastWords(words,targetWords=Pt){
  ...
  let cacheKey=`${config.words}:${normalized.join(" ")}`,cached=hodlLastWordCache.get(cacheKey);if(cached)return cached;
  ...
  for(let suffix=0;suffix<2**missingEntropyBits;suffix++){
    let entropyBits=prefixBits+suffix.toString(2).padStart(missingEntropyBits,"0"),bytes=new Uint8Array(config.bytes);for(let index=0;index<bytes.length;index++)bytes[index]=Number.parseInt(entropyBits.slice(index*8,index*8+8),2);
    let checksum=Z(bytes)[0]>>(8-checksumBits),wordIndex=suffix*2**checksumBits+checksum;candidates.push(Ae[wordIndex])
  }
  let result={partialCount:normalized.length,completeCount:config.words,candidates};if(hodlLastWordCache.size>=32)hodlLastWordCache.delete(hodlLastWordCache.keys().next().value);hodlLastWordCache.set(cacheKey,result);return result
}
function hodlSeedFinalWordContext(value,targetWords=Pt){
  ...
  let result=hodlComputeTargetLastWords(baseTokens.map(token=>token.word),config.words);if(!result||result.error||result.completeCount!==config.words)return null;
...
function hodlAnalyzeSeedInput(input,targetWords=Pt){
  let value=input.value,config=hodlSeedConfig(targetWords);if(hodlLooksExtendedKey(value))return{tokens:[],invalidRanges:[],invalidWords:[],excessCount:0,checksumInvalid:!1,extendedKey:!0,finalContext:null};
  ...
  return{tokens,invalidRanges,invalidWords,excessCount,checksumInvalid,extendedKey:!1,finalContext}
}
...
function hodlAutocompleteSeedInput(input,event,completeExisting=!1){
  let toggle=document.getElementById("seed-autocomplete");if(!toggle?.checked||!completeExisting&&(event?.inputType!=="insertText"||event.isComposing)||input.selectionStart!==input.selectionEnd)return!1;
...
function hodlKeyboardToggleMarkup(id,label){
  return`<button type="button" class="seed-keyboard-toggle" id="${id}" data-on-screen-keyboard-toggle aria-label="${hodlOnScreenKeyboardOpen?`Hide ${label}`:`Show ${label}`}" aria-controls="seed-keyboard" aria-expanded="${hodlOnScreenKeyboardOpen}"><svg viewBox="0 0 64 44" aria-hidden="true" focusable="false"><rect class="seed-keyboard-icon-case" x="3" y="6" width="58" height="32" rx="4"/><g class="seed-keyboard-icon-keys"><rect x="9" y="10" width="4" height="5" rx=".5"/><rect x="15" y="10" width="4" height="5" rx=".5"/><rect x="21" y="10" width="4" height="5" rx=".5"/><rect x="27" y="10" width="4" height="5" rx=".5"/><rect x="33" y="10" width="4" height="5" rx=".5"/><rect x="39" y="10" width="4" height="5" rx=".5"/><rect x="45" y="10" width="4" height="5" rx=".5"/><rect x="51" y="10" width="4" height="5" rx=".5"/><rect x="12" y="18" width="4" height="5" rx=".5"/><rect x="18" y="18" width="4" height="5" rx=".5"/><rect x="24" y="18" width="4" height="5" rx=".5"/><rect x="30" y="18" width="4" height="5" rx=".5"/><rect x="36" y="18" width="4" height="5" rx=".5"/><rect x="42" y="18" width="4" height="5" rx=".5"/><rect x="48" y="18" width="4" height="5" rx=".5"/><rect x="17" y="28" width="30" height="5" rx=".75"/></g></svg></button>`
}
function hodlSeedKeyboardToggleMarkup(){return hodlKeyboardToggleMarkup("seed-keyboard-toggle","on-screen seed keyboard")}
function hodlPassphraseKeyboardToggleMarkup(){return hodlKeyboardToggleMarkup("passphrase-keyboard-toggle","on-screen passphrase keyboard")}
function hodlPrivateKeyKeyboardToggleMarkup(){return`<div class="passphrase-keyboard-tools">${hodlKeyboardToggleMarkup("private-keyboard-toggle","on-screen private key keyboard")}</div>`}
var hodlSeedKeyboardLayouts={lower:["abcdefghij","klmnopqrs","tuvwxyz"],upper:["ABCDEFGHIJ","KLMNOPQRS","TUVWXYZ"],number:["1234567890","!@#$%^&*()","-_+=/?\\"]};
function hodlKeyboardMarkup(passphraseOnly=!1,inputName=passphraseOnly?"passphrase":"seed phrase"){
  let letters=hodlSeedKeyboardLayouts.lower.map((row,index)=>`<div class="seed-keyboard-row" data-seed-keyboard-row="${index+1}">${Array.from({length:hodlSeedKeyboardLayouts.number[index].length},(_,keyIndex)=>{let letter=row[keyIndex];return`<button type="button" class="seed-keyboard-key" data-seed-character-key${letter?` data-seed-key="${letter}" aria-label="Enter ${letter}"`:` hidden disabled aria-hidden="true"`}>${letter||""}</button>`}).join("")}${index===2?`<button type="button" class="seed-keyboard-key seed-keyboard-delete" data-seed-delete aria-label="Delete previous character"><svg viewBox="0 0 24 18" aria-hidden="true" focusable="false"><path d="M9 2h11a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H9L2 9l7-7Z"/><path d="m12 6 6 6m0-6-6 6"/></svg></button>`:""}</div>`).join("");
  return`<div class="seed-keyboard" id="seed-keyboard" data-on-screen-keyboard role="group" aria-label="On-screen lowercase ${inputName} keyboard" data-seed-keyboard-layout="lower"${hodlOnScreenKeyboardOpen?"":" hidden"}>${letters}<div class="seed-keyboard-space-row"><button type="button" class="seed-keyboard-mode" data-seed-keyboard-mode="lower" aria-label="${passphraseOnly?`Change ${inputName} character mode`:"Character mode switching is available for the passphrase"}"${passphraseOnly?"":" disabled"}>aA1</button><button type="button" class="seed-keyboard-space" data-seed-key=" " aria-label="Enter space">space</button></div></div>`
}
function hodlSeedKeyboardMarkup(){return hodlKeyboardMarkup(!1)}
function hodlPassphraseKeyboardMarkup(){return hodlKeyboardMarkup(!0)}
function hodlPrivateKeyKeyboardMarkup(){return hodlKeyboardMarkup(!0,"private key")}
function hodlSetOnScreenKeyboardOpen(open){
  hodlOnScreenKeyboardOpen=Boolean(open);document.querySelectorAll("[data-on-screen-keyboard-toggle]").forEach(toggle=>{toggle.setAttribute("aria-expanded",String(hodlOnScreenKeyboardOpen));let target=toggle.id==="passphrase-keyboard-toggle"?"passphrase":toggle.id==="private-keyboard-toggle"?"private key":"seed";toggle.setAttribute("aria-label",`${hodlOnScreenKeyboardOpen?"Hide":"Show"} on-screen ${target} keyboard`)});document.querySelectorAll("[data-on-screen-keyboard]").forEach(keyboard=>{keyboard.hidden=!hodlOnScreenKeyboardOpen})
}
function hodlSetSeedKeyboardLayout(keyboard,button,next){
  if(!keyboard||!button||!hodlSeedKeyboardLayouts[next])return;let layout=hodlSeedKeyboardLayouts[next];
  keyboard.querySelectorAll("[data-seed-keyboard-row]").forEach((row,index)=>{let keys=row.querySelectorAll("[data-seed-character-key]"),characters=[...layout[index]];keys.forEach((key,keyIndex)=>{let character=characters[keyIndex];key.hidden=!character;if(character){key.dataset.seedKey=character;key.textContent=character;key.setAttribute("aria-label",`Enter ${character}`);key.removeAttribute("aria-hidden")}else{delete key.dataset.seedKey;key.textContent="";key.disabled=!0;key.removeAttribute("aria-label");key.setAttribute("aria-hidden","true")}})});
  button.dataset.seedKeyboardMode=next;keyboard.dataset.seedKeyboardLayout=next;keyboard.setAttribute("aria-label",next==="lower"?"On-screen lowercase seed phrase keyboard":next==="upper"?"On-screen uppercase keyboard":"On-screen number and symbol keyboard")
}
function hodlCycleSeedKeyboardLayout(keyboard,button){
  if(!keyboard||!button)return;let order=["lower","upper","number"],current=button.dataset.seedKeyboardMode||"lower",next=order[(order.indexOf(current)+1)%order.length];hodlSetSeedKeyboardLayout(keyboard,button,next)
}
function hodlSeedKeyboardCanEnterCharacter(input,key,targetWords=Pt){
  let character=String(key??"").toLowerCase();if(!/^[a-z]$/.test(character))return!1;
  ...
  if(hodlLooksExtendedKey(value))return!1;
  let tokens=[...value.matchAll(/\S+/g)].map(match=>({word:match[0].toLowerCase(),start:match.index,end:match.index+match[0].length}));if(tokens.length>config.words)return!1;
  let tokenIndex=tokens.findIndex(token=>token.start<caret&&caret<=token.end);if(tokenIndex<0||tokenIndex>=config.words||tokens.slice(0,tokenIndex).some(token=>!hodlBip39WordSet.has(token.word)))return!1;
  let token=tokens[tokenIndex],options=Ae;if(tokenIndex===config.partialWords){let context=hodlSeedFinalWordContext(value,config.words);if(!context)return!1;options=context.candidates}
  return options.some(word=>word.startsWith(token.word))
}
function hodlSeedKeyboardCanEnterSpace(input,targetWords=Pt){
  let start=input.selectionStart??input.value.length,end=input.selectionEnd??start,config=hodlSeedConfig(targetWords);if(start!==end||end!==input.value.length||!end||/\s$/.test(input.value))return!1;
  let words=input.value.trim().toLowerCase().split(/\s+/).filter(Boolean);return words.length<config.words&&words.every(word=>hodlBip39WordSet.has(word))
}
function hodlUpdateSeedKeyboardKeys(input,targetWords=Pt){
  let keyboard=document.getElementById("seed-keyboard");if(!keyboard||!input)return;
  keyboard.querySelectorAll("[data-seed-character-key]").forEach(button=>{button.disabled=!hodlSeedKeyboardCanEnterCharacter(input,button.dataset.seedKey,targetWords)});
  let space=keyboard.querySelector(".seed-keyboard-space");if(space)space.disabled=!hodlSeedKeyboardCanEnterSpace(input,targetWords);
  let remove=keyboard.querySelector("[data-seed-delete]"),start=input.selectionStart??input.value.length,end=input.selectionEnd??start;if(remove)remove.disabled=start===end&&start===0
}
function hodlUpdatePassphraseKeyboardKeys(input){
  let keyboard=document.getElementById("seed-keyboard");if(!keyboard||!input)return;keyboard.querySelectorAll("[data-seed-character-key]").forEach(button=>{button.disabled=!1});
  let space=keyboard.querySelector(".seed-keyboard-space");if(space)space.disabled=!1;let remove=keyboard.querySelector("[data-seed-delete]"),start=input.selectionStart??input.value.length,end=input.selectionEnd??start;if(remove)remove.disabled=start===end&&start===0
}
function hodlKeyboardValueAfterInsert(input,key){let start=input.selectionStart??input.value.length,end=input.selectionEnd??start;return input.value.slice(0,start)+String(key??"")+input.value.slice(end)}
function hodlHexPrivateKeyPrefix(value){
  let candidate=String(value??""),prefixed=/^0[xX]/.test(candidate),body=prefixed?candidate.slice(2):candidate;if(!prefixed&&/[xX]/.test(candidate)||!/^[0-9a-fA-F]*$/.test(body)||body.length>64)return!1;if(body.length<64)return!0;
  try{hf(M.decode(body.toLowerCase()));return!0}catch{return!1}
}
function hodlWifPrivateKeyPrefix(value,network){
  let candidate=String(value??""),first=candidate[0]||"",prefixes=network==="testnet"?["9","c"]:["5","K","L"];if(!prefixes.includes(first)||!/^[1-9A-HJ-NP-Za-km-z]+$/.test(candidate))return!1;
  let expected=first==="5"||first==="9"?51:52;if(candidate.length>expected)return!1;if(candidate.length<expected)return!0;
  try{let decoded=Ls(candidate);return decoded.network===network&&Boolean(decoded.priv)}catch{return!1}
}
function hodlMiniPrivateKeyPrefix(value){
  let candidate=String(value??"");if(candidate.length>30||!/^S[1-9A-HJ-NP-Za-km-z]*$/.test(candidate))return!1;if(candidate.length<30)return!0;return $o(candidate)
}
...
function hodlUpdatePrivateKeyInputPresentation(){
  let input=document.getElementById("key");if(!input)return;let kind=hodlNormalizePrivateKeyKind(document.querySelector('input[name="kk"]:checked')?.value,input.value),network=hodlSelectedNetwork(document.getElementById("network"));input.placeholder=hodlPrivateKeyPlaceholder(kind,network);input.setAttribute("inputmode",kind==="hex-key"?"text":"text");input.setAttribute("autocapitalize","off");input.setAttribute("autocomplete","off");input.setAttribute("spellcheck","false")
}
function hodlPrivateKeyboardCanEnterCharacter(input,key){
  let candidate=hodlKeyboardValueAfterInsert(input,key),kind=hodlNormalizePrivateKeyKind(document.querySelector('input[name="kk"]:checked')?.value,input.value);if(kind==="brain")return!0;if(kind==="minikey")return hodlMiniPrivateKeyPrefix(candidate);if(kind==="hex-key")return hodlHexPrivateKeyPrefix(candidate);
...
function hodlUpdatePrivateKeyKeyboardKeys(input){
  let keyboard=document.getElementById("seed-keyboard");if(!keyboard||!input)return;let kind=hodlNormalizePrivateKeyKind(document.querySelector('input[name="kk"]:checked')?.value,input.value);
  keyboard.querySelectorAll("[data-seed-character-key]").forEach(button=>{button.disabled=!hodlPrivateKeyboardCanEnterCharacter(input,button.dataset.seedKey)});let space=keyboard.querySelector(".seed-keyboard-space");if(space)space.disabled=kind!=="brain";
  let remove=keyboard.querySelector("[data-seed-delete]"),start=input.selectionStart??input.value.length,end=input.selectionEnd??start;if(remove)remove.disabled=start===end&&start===0
}
function hodlApplySeedKeyboardKey(input,key,deleteBackward=!1){
  if(!input)return;let start=input.selectionStart??input.value.length,end=input.selectionEnd??start,inputType="insertText",data=key;
  if(deleteBackward){inputType="deleteContentBackward";data=null;if(start===end&&start>0)start-=1;input.setRangeText("",start,end,"end")}else input.setRangeText(String(key??""),start,end,"end");
  let event=typeof InputEvent==="function"?new InputEvent("input",{bubbles:!0,inputType,data}):new Event("input",{bubbles:!0});input.dispatchEvent(event);input.focus({preventScroll:!0})
}
function hodlBindSeedKeyboardDelete(getInput,button){
  if(typeof getInput!=="function"||!button)return;let holdTimer=null,repeatTimer=null,repeated=!1,pointerId=null;
  let stop=()=>{if(holdTimer!==null){clearTimeout(holdTimer);holdTimer=null}if(repeatTimer!==null){clearInterval(repeatTimer);repeatTimer=null}let captured=pointerId;pointerId=null;try{if(captured!==null&&button.hasPointerCapture?.(captured))button.releasePointerCapture(captured)}catch{}};
  let remove=()=>{let input=getInput();if(!input||button.disabled){stop();return}hodlApplySeedKeyboardKey(input,"",!0)};
  button.addEventListener("pointerdown",event=>{if(event.button!==0||button.disabled)return;stop();repeated=!1;pointerId=event.pointerId;try{button.setPointerCapture?.(event.pointerId)}catch{}holdTimer=setTimeout(()=>{holdTimer=null;repeated=!0;remove();if(!button.disabled)repeatTimer=setInterval(remove,69)},420)});
  ["pointerup","pointercancel","pointerleave","lostpointercapture"].forEach(type=>button.addEventListener(type,stop));
  button.addEventListener("click",event=>{if(repeated){event.preventDefault();repeated=!1;return}remove()})
}
function hodlBindSeedKeyboard(input,targetWords=Pt){
  let toggle=document.getElementById("seed-keyboard-toggle"),keyboard=document.getElementById("seed-keyboard"),modeButton=keyboard?.querySelector("[data-seed-keyboard-mode]"),passphrase=document.getElementById("pass");if(!toggle||!keyboard||!input)return;
  let activeInput=input,isPassphrase=()=>Boolean(passphrase&&activeInput===passphrase),refresh=()=>{if(isPassphrase())hodlUpdatePassphraseKeyboardKeys(activeInput);else hodlUpdateSeedKeyboardKeys(input,targetWords)};
  let activate=target=>{activeInput=target;let pass=isPassphrase();if(modeButton){if(!pass&&modeButton.dataset.seedKeyboardMode!=="lower")hodlSetSeedKeyboardLayout(keyboard,modeButton,"lower");modeButton.disabled=!pass;modeButton.setAttribute("aria-label",pass?"Change passphrase character mode":"Character mode switching is available for the passphrase")}keyboard.setAttribute("aria-label",pass?`On-screen ${keyboard.dataset.seedKeyboardLayout||"lower"} passphrase keyboard`:"On-screen lowercase seed phrase keyboard");refresh()};
  toggle.onclick=()=>{hodlSetOnScreenKeyboardOpen(!hodlOnScreenKeyboardOpen);refresh()};
  keyboard.querySelectorAll("button").forEach(button=>{button.addEventListener("pointerdown",event=>{event.preventDefault();activeInput.focus({preventScroll:!0})})});
  keyboard.querySelectorAll("[data-seed-character-key],.seed-keyboard-space").forEach(button=>{button.onclick=()=>hodlApplySeedKeyboardKey(activeInput,button.dataset.seedKey||"")});hodlBindSeedKeyboardDelete(()=>activeInput,keyboard.querySelector("[data-seed-delete]"));if(modeButton)modeButton.onclick=()=>{hodlCycleSeedKeyboardLayout(keyboard,modeButton);keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout} passphrase keyboard`);refresh()};
  [input,...(passphrase?[passphrase]:[])].forEach(field=>{field.addEventListener("focus",()=>activate(field));["input","click","keyup","select"].forEach(type=>field.addEventListener(type,refresh))});activate(input)
}
function hodlBindPassphraseKeyboard(inputId="pass",toggleId="passphrase-keyboard-toggle",inputName="passphrase"){
  let toggle=document.getElementById(toggleId),keyboard=document.getElementById("seed-keyboard"),input=document.getElementById(inputId),modeButton=keyboard?.querySelector("[data-seed-keyboard-mode]");if(!toggle||!keyboard||!input)return;
  let privateKey=inputId==="key",refresh=()=>privateKey?hodlUpdatePrivateKeyKeyboardKeys(input):hodlUpdatePassphraseKeyboardKeys(input);toggle.onclick=()=>{hodlSetOnScreenKeyboardOpen(!hodlOnScreenKeyboardOpen);refresh()};keyboard.querySelectorAll("button").forEach(button=>{button.addEventListener("pointerdown",event=>{event.preventDefault();input.focus({preventScroll:!0})})});
  keyboard.querySelectorAll("[data-seed-character-key],.seed-keyboard-space").forEach(button=>{button.onclick=()=>hodlApplySeedKeyboardKey(input,button.dataset.seedKey||"")});hodlBindSeedKeyboardDelete(()=>input,keyboard.querySelector("[data-seed-delete]"));if(modeButton){modeButton.disabled=!1;modeButton.onclick=()=>{hodlCycleSeedKeyboardLayout(keyboard,modeButton);keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout} ${inputName} keyboard`);refresh()}};
  ["input","focus","click","keyup","select"].forEach(type=>input.addEventListener(type,refresh));if(privateKey){document.querySelectorAll('input[name="kk"]').forEach(radio=>radio.addEventListener("change",refresh));document.getElementById("network")?.addEventListener("change",refresh)}refresh()
}
function hodlRenderPassphraseKeyboard(){
  let host=document.getElementById("passphrase-keyboard-host"),toggleHost=document.getElementById("passphrase-keyboard-toggle-host"),privateKey=Ne==="key",passphrase=Ne==="dice"||Ne==="hex",enabled=passphrase||privateKey;if(toggleHost){toggleHost.hidden=!passphrase;toggleHost.innerHTML=passphrase?hodlPassphraseKeyboardToggleMarkup():""}if(!host)return;host.hidden=!enabled;host.innerHTML=enabled?(privateKey?hodlPrivateKeyKeyboardMarkup():hodlPassphraseKeyboardMarkup()):"";if(enabled)hodlBindPassphraseKeyboard(privateKey?"key":"pass",privateKey?"private-keyboard-toggle":"passphrase-keyboard-toggle",privateKey?"private key":"passphrase")
}
...
function hodlRenderKeyForm(){
  let config=hodlSeedConfig(),keyboardHost=document.getElementById("passphrase-keyboard-host"),toggleHost=document.getElementById("passphrase-keyboard-toggle-host");if(keyboardHost){keyboardHost.hidden=!0;keyboardHost.innerHTML=""}if(toggleHost){toggleHost.hidden=!0;toggleHost.innerHTML=""}hodlUpdateSeedLengthControl();
  if(Ne==="dice"){
    ...
    hodlBindKeyFields();hodlRenderPassphraseKeyboard();return
  }
  if(Ne==="cards"){
    let needed=hodlCardNeeded(config.words),suitPad=hodlCardSuits.map(suit=>`<button type="button" class="card-suit${suit.red?" is-red":""}${suit.code===hodlCardSuit?" active":""}" data-card-suit="${suit.code}" aria-label="${suit.label}" aria-pressed="${suit.code===hodlCardSuit}">${suit.symbol}</button>`).join("");
    ...
    at.innerHTML=`
      ...
      <p class="muted" id="cards-help">${config.words===24?"Shuffle thoroughly, deal all 52 unique cards, then shuffle again and deal 6 more. SHA-256 of the ASCII transcript becomes the 24-word seed.":`Shuffle thoroughly and deal ${needed.first} unique cards without putting them back. SHA-256 of the ASCII transcript (AS 2C TD) becomes the ${config.words}-word seed.`}</p>
      ...
      <div class="dealt-cards" id="dealt-cards" aria-live="polite"></div>
      <aside class="cards-reshuffle" id="cards-reshuffle" hidden></aside>
      <label class="field">Card transcript
    ...
    input.oninput=()=>hodlUpdateCards();
    at.querySelectorAll("[data-card-suit]").forEach(button=>{button.onclick=()=>{hodlCardSuit=button.getAttribute("data-card-suit");hodlUpdateCards()}});
    at.querySelectorAll("[data-card-rank]").forEach(button=>{button.onclick=()=>hodlAppendCard(button.getAttribute("data-card-rank"))});
    document.getElementById("card-undo").onclick=hodlUndoCard;
    hodlBindKeyFields();hodlUpdateCards();return
  }
  ...
  if(Ne==="hex"){
    ...
    hodlBindKeyFields();let entropyInput=document.getElementById(inputId);if(entropyInput)at.querySelectorAll("[data-entropy-digit]").forEach(button=>{button.onclick=()=>hodlInsertEntropyControl(entropyInput,button)});hodlRenderPassphraseKeyboard();return
  }
  if(Ne==="seed"){
    ...
    let toggle=document.getElementById("seed-autocomplete");toggle.onchange=()=>{let state=hodlKeys[hodlActiveKey];if(state)state.seedAutocomplete=toggle.checked;input.focus({preventScroll:!0});if(toggle.checked&&hodlAutocompleteSeedInput(input,null,!0)){let event=typeof InputEvent==="function"?new InputEvent("input",{bubbles:!0,inputType:"insertReplacementText",data:null}):new Event("input",{bubbles:!0});input.dispatchEvent(event)}else update()};
    input.oninput=event=>{hodlApplyFilteredInput(input,hodlFilterSeed);hodlAutocompleteSeedInput(input,event);update()};input.onscroll=()=>hodlSyncDiceHighlight(input);input.onfocus=update;input.onblur=event=>{if(!event.relatedTarget?.closest?.("#seed-keyboard,.seed-autocomplete-toggle"))update()};hodlBindSeedKeyboard(input,config.words);hodlBindKeyFields();update();return
  }
  at.innerHTML=`
  ...
  hodlBindKeyFields();hodlRenderPassphraseKeyboard()
}
...
function hodlPrivateKeyCharacterEntries(value){
  let entries=[];for(let index=0;index<String(value??"").length;){let character=String.fromCodePoint(String(value).codePointAt(index)),end=index+character.length;if(!/\s/.test(character))entries.push({character,start:index,end});index=end}return entries
}
function hodlPrivateKeyInputAnalysis(value,kind,network){
  let selected=hodlNormalizePrivateKeyKind(kind,value),entries=hodlPrivateKeyCharacterEntries(value),invalidRanges=[],ready=!1,status="",first=entries[0],last=entries.at(-1),markAll=()=>{if(first&&last)invalidRanges.push([first.start,last.end])};
  if(selected==="brain")return{invalidRanges,ready:Boolean(String(value??"").length),status:String(value??"").length?"Recovery passphrase entered · brain wallets are unsafe · recovery only":"No recovery passphrase entered · brain wallets are unsafe · recovery only",kind:selected};
  if(selected==="hex-key"){
    let prefixed=entries[0]?.character==="0"&&/^x$/i.test(entries[1]?.character||""),characters=entries.slice(prefixed?2:0),valid=characters.filter(entry=>/^[0-9a-fA-F]$/.test(entry.character)),invalid=characters.filter(entry=>!/^[0-9a-fA-F]$/.test(entry.character)),excess=valid.slice(64);invalidRanges.push(...invalid.map(entry=>[entry.start,entry.end]),...excess.map(entry=>[entry.start,entry.end]));
    let count=valid.length,remaining=Math.max(0,64-count),parts=[count>64?`${count} hexadecimal characters entered · 64 required`:`${count} of 64 hexadecimal characters entered · ${remaining} remaining`];if(invalid.length)parts.push(`${invalid.length} invalid character${invalid.length===1?"":"s"} highlighted · use only 0–9 and a–f`);if(excess.length)parts.push(`${excess.length} extra highlighted · remove to continue`);
    if(!invalid.length&&!excess.length&&count===64)try{hodlAssertPrivateKeyKind(value,network,selected);ready=!0;parts=["64 of 64 hexadecimal characters entered","valid secp256k1 private key","ready to derive"]}catch(error){markAll();parts.push(error.message||"Invalid private key")}
    status=parts.join(" \u00b7 ");return{invalidRanges,ready,status,kind:selected,count,required:64,remaining}
  }
  if(selected==="wif"){
    let alphabet=/^[1-9A-HJ-NP-Za-km-z]$/,prefixes=network==="testnet"?["9","c"]:["5","K","L"],invalid=entries.filter(entry=>!alphabet.test(entry.character));if(first&&!prefixes.includes(first.character)&&!invalid.includes(first))invalid.push(first);
    let required=first&&["5","9"].includes(first.character)?51:first&&["K","L","c"].includes(first.character)?52:null,count=entries.length,excess=required?entries.slice(required):[];invalidRanges.push(...invalid.map(entry=>[entry.start,entry.end]),...excess.map(entry=>[entry.start,entry.end]));
    let parts=[required?count>required?`${count} WIF characters entered · ${required} required`:`${count} of ${required} WIF characters entered · ${Math.max(0,required-count)} remaining`:`${count} of 51 or 52 WIF characters entered · starts with ${network==="testnet"?"9 or c":"5, K, or L"}`];if(invalid.length)parts.push(`${invalid.length} invalid character${invalid.length===1?"":"s"} highlighted · use ${network} Base58 WIF characters`);if(excess.length)parts.push(`${excess.length} extra highlighted · remove to continue`);
    if(required&&count===required&&!invalid.length&&!excess.length)try{hodlAssertPrivateKeyKind(value,network,selected);ready=!0;parts=[`${required} of ${required} WIF characters entered`,`${network} checksum valid`,`ready to derive`]}catch(error){markAll();parts.push(error.message||"Invalid WIF checksum")}
    status=parts.join(" \u00b7 ");return{invalidRanges,ready,status,kind:selected,count,required,remaining:required?Math.max(0,required-count):null}
  }
  let invalid=entries.filter((entry,index)=>index===0?entry.character!=="S":!/^[1-9A-HJ-NP-Za-km-z]$/.test(entry.character)),count=entries.length,required=count<=22?22:30,excess=entries.slice(30);invalidRanges.push(...invalid.map(entry=>[entry.start,entry.end]),...excess.map(entry=>[entry.start,entry.end]));let parts=[count>30?`${count} Mini-key characters entered · 30 maximum`:`${count} of ${required} Mini-key characters entered · ${Math.max(0,required-count)} remaining`];if(!count)parts=["0 of 22 or 30 Mini-key characters entered · must start with S"];if(invalid.length)parts.push(`${invalid.length} invalid character${invalid.length===1?"":"s"} highlighted · use S followed by Bitcoin Base58 characters`);if(excess.length)parts.push(`${excess.length} extra highlighted · remove to continue`);
  if((count===22||count===30)&&!invalid.length&&!excess.length)try{hodlAssertPrivateKeyKind(value,network,selected);ready=!0;parts=[`${count} of ${count} Mini-key characters entered`,`checksum valid`,`ready to derive`]}catch(error){markAll();parts.push(error.message||"Invalid Mini-key checksum")}
  status=parts.join(" \u00b7 ");return{invalidRanges,ready,status,kind:selected,count,required,remaining:Math.max(0,required-count)}
}
function hodlRenderPrivateKeyInputState(input){
  if(!input)return null;let kind=hodlNormalizePrivateKeyKind(document.querySelector('input[name="kk"]:checked')?.value,input.value),network=hodlSelectedNetwork(document.getElementById("network")),analysis=hodlPrivateKeyInputAnalysis(input.value,kind,network),meta=document.getElementById("private-key-meta"),invalid=analysis.invalidRanges.length>0;input.classList.toggle("bad",invalid);input.setAttribute("aria-invalid",String(invalid));hodlRenderInputHighlight(input,analysis.invalidRanges);if(meta){meta.textContent=analysis.status;meta.className="muted"+(analysis.ready?" ok":invalid||kind==="brain"&&input.value.length?" err":"")}return analysis
}
function hodlBindKeyFields(){
  ...
  let key=document.getElementById("key");if(key){let apply=event=>{let selected=document.querySelector("input[name=kk]:checked"),kind=hodlNormalizePrivateKeyKind(selected?.value,key.value),pasted=event?.inputType==="insertFromPaste";if(pasted&&kind!=="brain"){let detected=hodlDetectPrivateKeyKind(key.value);if(detected&&detected!==kind){let radio=document.querySelector(`input[name="kk"][value="${detected}"]`);if(radio){radio.checked=!0;kind=detected}}}if(kind!=="brain")key.value=hodlFilterKey(key.value,kind);hodlUpdatePrivateKeyInputPresentation();hodlRenderPrivateKeyInputState(key);hodlUpdatePrivateKeyKeyboardKeys(key)};key.oninput=apply;key.onscroll=()=>hodlSyncDiceHighlight(key);document.querySelectorAll("input[name=kk]").forEach(radio=>radio.addEventListener("change",apply));document.getElementById("network")?.addEventListener("change",apply);apply()}
}
...
function hodlPrivateKeyInputIsValid(){
  ...
  try{hodlAssertPrivateKeyKind(value,hodlSelectedNetwork(document.getElementById("network")),kind);return!0}catch{return!1}
}
function hodlCanDeriveCurrentKey(){
  try{
    ...
    if(Ne==="cards"){let input=document.getElementById("cards");return input?hodlCardsEntropy(input.value,Pt).ok:!1}
...
function hodlFingerprintMnemonic(){
  try{
    ...
    if(Ne==="cards"){let input=document.getElementById("cards");if(!input)return null;let entropy=hodlCardsEntropy(input.value,Pt);return entropy.ok?_n(entropy.bytes):null}
    if(Ne==="hex"){
...
function hodlInitMasterFingerprintPreview(){
  ...
  panel.addEventListener("input",event=>{
    let id=event.target?.id;if(!["pass","dice","hex","bin","seed","cards"].includes(id))return;
  ...
  panel.addEventListener("click",event=>{
    let target=event.target instanceof Element?event.target.closest("#modes button, [data-seed-words], [data-d], [data-lw], [data-card-suit], [data-card-rank], #card-undo"):null;if(!target)return;
    hodlInvalidateLiveKeyResult();hodlQueueMasterFingerprintPreview()
  });
...
function hodlCalculateKey(){
  ...
  try{
    ...
    if(Ne==="dice"){
      ...
      let entropy=hodlCardsEntropy(document.getElementById("cards").value,Pt);if(!entropy.ok)throw new Error(entropy.error);re=on(entropy,passphrase,network,count,account)
    }else if(Ne==="hex"){
    ...
    }else{let value=document.getElementById("key").value,kind=hodlNormalizePrivateKeyKind(document.querySelector("input[name=kk]:checked")?.value,value);hodlAssertPrivateKeyKind(value,network,kind);re=Io(value,network,kind)}
...
function hodlFilterSeed(e){let value=String(e??"").replace(/[^a-zA-Z0-9\s]/g,"");return hodlLooksExtendedKey(value)?value:value.toLowerCase()}
function hodlFilterKey(e,t){return t==="brain"?e:e.replace(/[^0-9A-Za-z\s]/g,"")}
function hodlDecodeMiniPrivateKey(value){let candidate=String(value??"").trim();if(!/^S(?:[1-9A-HJ-NP-Za-km-z]{21}|[1-9A-HJ-NP-Za-km-z]{29})$/.test(candidate))throw new Error("Mini keys must start with S and contain 22 or 30 Bitcoin Base58 characters.");return Ns(candidate)}
function hodlAssertPrivateKeyKind(value,network,kind){
  let candidate=String(value??"").trim(),selected=hodlNormalizePrivateKeyKind(kind,candidate);if(!candidate)throw new Error(selected==="brain"?"Enter the brain-wallet recovery passphrase.":"Enter a private key.");
  if(selected==="brain")return candidate;
  if(selected==="minikey"){hf(hodlDecodeMiniPrivateKey(candidate));return candidate}
  if(selected==="hex-key"){
    let compact=candidate.replace(/\s/g,"").replace(/^0x/i,"");if(!/^[0-9a-fA-F]{64}$/.test(compact))throw new Error("Enter exactly 64 hexadecimal characters (0–9 and a–f).");hf(M.decode(compact.toLowerCase()));return compact.toLowerCase()
  }
  let decoded;try{decoded=Ls(candidate)}catch{throw new Error(`Enter a valid ${network} WIF private key (${network==="testnet"?"9… or c…":"5…, K…, or L…"}).`)}if(decoded.network!==network)throw new Error(`This WIF is for ${decoded.network}; Network is set to ${network}.`);hf(decoded.priv);return candidate
}
...
function hodlSummarizeMultisigAccounts(accountNumbers){
  let accounts=[...new Set(accountNumbers.filter(account=>Number.isSafeInteger(account)&&account>=0&&account<=0x7fffffff))].sort((a,b)=>a-b);
  let mixed=accounts.length>1;return{account:accounts.length===1?accounts[0]:null,accounts,consistent:!mixed,mixed}
}
...
function hodlDetectMsigScriptSummary(values=hodlReadMsigXpubs()){
  ...
  for(let raw of values){
    ...
    try{let evidence=hodlMultisigScriptEvidence(hodlParseMultisigCosigner(raw));if(evidence.prefixKind)kinds.push(evidence.prefixKind);if(evidence.originKind)kinds.push(evidence.originKind);if(evidence.standard==="bip45"||evidence.standard==="bip87")legacyStandards.push(evidence.standard)}catch{}
  }
...
function hodlUpdateMsigLegacyControls(){
  ...
  if(option)option.textContent=checkbox?.checked?"Legacy · BIP87":"Legacy · BIP45"
}
function hodlMultisigKeyPlaceholder(kind,network,legacyStandard="bip45"){
  let testnet=network==="testnet",coin=testnet?"1h":"0h";
...
function hodlUpdateMsigKeyPlaceholders(){
  ...
  document.querySelectorAll("#msig-keys textarea").forEach(textarea=>{textarea.placeholder=placeholder})
}
function hodlSyncMsigDeriveButton(){
  let button=document.getElementById("msig-go");if(!button)return;
  let ready=!1,reason="";try{hodlValidatedMsigInputs();ready=!0}catch(error){reason=error.message||"Complete every multisig field."}
  button.disabled=!ready;button.setAttribute("aria-disabled",String(!ready));button.title=ready?"":reason
}
function hodlUpdateMsigScriptDetection(){
  let select=document.getElementById("msig-script-type");if(!select)return hodlSummarizeMultisigScriptKinds([]);
  ...
  if(desired==="mixed"){
    ...
    select.dataset.lastConcrete=desired;hodlSyncSelect(select,desired)
  ...
  if(warning){warning.textContent=message;warning.hidden=!message}
  hodlUpdateMsigKeyPlaceholders();hodlSyncMsigDeriveButton();return summary
}
...
function hodlUpdateMsigAccount(){
  let field=document.getElementById("msig-account");if(!field)return hodlSummarizeMultisigAccounts([]);
  ...
  if(kind==="p2sh"&&legacyStandard==="bip45"){
    field.value="";field.placeholder="Not applicable";field.dataset.state="not-applicable";
    if(help)help.textContent="BIP45 purpose keys do not contain an account number.";
    if(warning){warning.textContent="";warning.hidden=!0}
    return hodlSummarizeMultisigAccounts([])
  }
  let accountNumbers=[];
  for(let raw of hodlReadMsigXpubs()){
    if(!raw.trim())continue;
    try{let parsed=hodlParseMultisigCosigner(raw.trim());if(parsed.origin)accountNumbers.push(hodlMultisigAccountNumber(parsed.origin,kind))}catch{}
  }
  let summary=hodlSummarizeMultisigAccounts(accountNumbers),message=hodlMultisigAccountWarning(summary);
  field.value=summary.mixed?"Mixed":summary.account==null?"":String(summary.account);field.placeholder="Derived from keys";field.dataset.state=summary.mixed?"mixed":summary.account==null?"empty":"account";
...
function hodlInvalidateMsig(){let state=hodlMsigs[hodlActiveMsig];if(state){state.result=null;state.error=""}re=null;dr.innerHTML="";let err=document.getElementById("msig-error");if(err)err.textContent="";hodlUpdateMsigAccount();hodlSyncMsigDeriveButton()}
function hodlUpdateMsigHint(){let n=Number(document.getElementById("msig-n").value||3),m=document.getElementById("msig-m").value||"2",hint=document.getElementById("msig-hint");if(hint){hint.textContent=n===1?"Spending will need this key. Receiving needs none of the private keys.":`Spending will need ${m} of these ${n} keys. Receiving needs none of the private keys.`;hint.className="hint ok"}}
var hodlMsigSliderBaseMax=9,hodlMsigSliderLimit=15;
function hodlClampMsigThreshold(value,min,max){let number=Number(value);return Math.min(max,Math.max(min,Number.isFinite(number)?Math.round(number):min))}
function hodlRenderMsigThreshold(){
  ...
  slider.style.setProperty("--msig-m-position",(m-1)/span*100+"%");slider.style.setProperty("--msig-n-position",(n-1)/span*100+"%");
  slider.dataset.sliderMax=String(visibleMax);slider.dataset.overlap=String(m===n);
  let mNumber=document.getElementById("msig-m-number"),nNumber=document.getElementById("msig-n-number");
  if(mNumber){mNumber.value=String(m);mNumber.min="1";mNumber.max=String(hodlMsigSliderLimit)}if(nNumber){nNumber.value=String(n);nNumber.min="1";nNumber.max=String(hodlMsigSliderLimit)}
  mInput.setAttribute("aria-valuetext",m+" signature"+(m===1?"":"s")+" needed");nInput.setAttribute("aria-valuetext",n+" total signing key"+(n===1?"":"s"));
  let fragment=document.createDocumentFragment();for(let value=1;value<=visibleMax;value++){let tick=document.createElement("span");tick.textContent=String(value);tick.style.setProperty("--msig-tick-position",(value-1)/span*100+"%");fragment.appendChild(tick)}ticks.replaceChildren(fragment)
}
function hodlSetMsigThresholds(mValue,nValue,changed,moveOther){
  let mInput=document.getElementById("msig-m"),nInput=document.getElementById("msig-n");if(!mInput||!nInput)return{m:2,n:3};
  let n=hodlClampMsigThreshold(nValue,1,hodlMsigSliderLimit),m=hodlClampMsigThreshold(mValue,1,hodlMsigSliderLimit);
  if(moveOther){if(changed==="m")n=Math.max(n,m);else if(changed==="n")m=Math.min(m,n)}else if(changed==="n")n=Math.max(n,m);else m=Math.min(m,n);
  mInput.value=String(m);nInput.value=String(n);hodlRenderMsigThreshold();hodlUpdateMsigHint();return{m,n}
}
function hodlChangeMsigThreshold(handle,value,moveOther){
  ...
  if(next.n!==previousN)hodlFillKeys(saved);else hodlUpdateMsigHint();hodlInvalidateMsig()
}
...
function hodlBindMsigThresholdSlider(){
  ...
  slider.addEventListener("pointerdown",event=>{
    if(event.button!==0)return;event.preventDefault();let rect=slider.getBoundingClientRect(),m=Number(mInput.value),n=Number(nInput.value),visibleMax=Math.max(hodlMsigSliderBaseMax,n),point=hodlMsigThresholdPointerValue(event.clientX,rect,visibleMax),handle=m===n?null:Math.abs(point-m)<=Math.abs(point-n)?"m":"n";
    drag={pointerId:event.pointerId,startX:event.clientX,rect,visibleMax,handle};slider.setPointerCapture(event.pointerId);if(handle)setActive(handle,point)
  });
  slider.addEventListener("pointermove",event=>{
    if(!drag||event.pointerId!==drag.pointerId)return;event.preventDefault();if(!drag.handle){let delta=event.clientX-drag.startX;if(Math.abs(delta)<3)return;drag.handle=delta<0?"m":"n"}
...
function hodlMultisigAccountKeyError(parsed,kind,legacyStandard="bip45"){
  ...
  if(legacyStandard==="bip87"){
    ...
    if(parsed.childNumber<0x80000000)return"A BIP87 account index must be hardened.";
    return""
  }
  if(parsed.depth!==1)return`Legacy P2SH requires the depth-1 BIP45 purpose key at m/45h; this key is depth ${parsed.depth}.`;
  if(parsed.childNumber!==0x8000002d)return"Legacy P2SH requires the hardened BIP45 purpose child at m/45h.";
  return""
}
...
function hodlValidatedMsigInputs(){
  ...
  for(let index=0;index<n;index++){
    let field=document.getElementById("msig-x-"+index),raw=field?.value.trim()||"";if(!raw)throw new Error("Paste an origin and extended public key for co-signer "+(index+1)+".");
    ...
    let accountError=hodlMultisigAccountKeyError(parsed,kind,legacyStandard);if(accountError)throw new Error(`Co-signer ${index+1}: ${accountError}`);
    ...
    let scriptOriginError=hodlOriginScriptError(parsed.origin,kind,network,legacyStandard);if(scriptOriginError)throw new Error(`Co-signer ${index+1}: ${scriptOriginError}`);
    let accountNumber=hodlMultisigAccountNumber(parsed.origin,kind);if(accountNumber!=null)accountNumbers.push(accountNumber);
    let node=parsed.node,canonical=hodlCanonicalMultisigKey(parsed);if(xpubs.includes(canonical))throw new Error(`Co-signer ${index+1} duplicates an earlier co-signer. Every slot must use a distinct extended public key.`);
...
function hodlRfc6979Compare(sighash,privateKey,r){
  let plain=xe.sign(sighash,privateKey,{prehash:!1,extraEntropy:!1});
  if(hodlEq(plain.slice(0,32),r)){
    return hodlIsLowR(r)
      ?{ok:!0,className:"psbt-ok",message:"Matches RFC 6979 (plain deterministic nonce)."}
      :{ok:!0,className:"psbt-ok",message:"Matches RFC 6979 (plain deterministic nonce). r is high; Bitcoin Core would grind this one."};
  }
  for(let n=1;n<=64;n++){
    let expected=xe.sign(sighash,privateKey,{prehash:!1,extraEntropy:hodlLe32Counter(n)});
    if(hodlEq(expected.slice(0,32),r)){
      return {ok:!0,className:"psbt-ok",message:"Matches RFC 6979 with Bitcoin Core-style low-r grind (retry "+n+"). Saves one byte. Not a leak."};
    }
  }
  let zeros=xe.sign(sighash,privateKey,{prehash:!1,extraEntropy:new Uint8Array(32)});
  if(hodlEq(zeros.slice(0,32),r)){
    return {ok:!0,className:"psbt-ok",message:"Matches RFC 6979 with 32 zero extra-entropy bytes (some libraries mix this in)."};
  }
...
function hodlRestoreFormFields(state){
  ...
  ["dice","hex","bin","seed","key","cards"].forEach(id=>{let el=document.getElementById(id);if(el){el.value=id==="dice"&&ge==="dplus"?state.fields.dplusDice||"":state.fields[id]||"";if(id==="dice"){el.dataset.previousValue=el.value;el.setSelectionRange(el.value.length,el.value.length)}el.dispatchEvent(new Event("input"))}});
}
function hodlSetMode(mode){
  hodlCaptureKey();
  let state=hodlKeys[hodlActiveKey];if(state)state.mode=mode;
  Ne=mode;hodlEntropyFormat=state?.entropyFormat==="bin"?"bin":"hex";[...Zs.children].forEach((button,index)=>{let active=hodlKeyModes[index]===Ne;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))});
...
function hodlKeyStateNeedsClear(state){
  ...
  return String(state.mode??"dice")!=="dice"||String(state.diceMethod??"coldcard")!=="coldcard"||String(state.entropyFormat??"hex")!=="hex"||Boolean(state.seedAutocomplete)||Boolean(state.dplusNumberedD16)||Number(state.targetWords??24)!==24||
    ...
    hodlNormalizePrivateKeyKind(fields.keyKind,fields.key)!=="wif"||["pass","dice","dplusDice","hex","bin","cards","seed","key"].some(hasText)
}
...
function hodlCaptureKey(){
  ...
  ["pass","account","count","hex","bin","seed","key","cards"].forEach(id=>{let el=document.getElementById(id);if(el)state.fields[id]=el.value});state.fields.network=hodlSelectedNetwork(document.getElementById("network"));
  let dice=document.getElementById("dice");if(dice)state.fields[ge==="dplus"?"dplusDice":"dice"]=dice.value;
  state.fields.keyKind=hodlNormalizePrivateKeyKind(document.querySelector("input[name=kk]:checked")?.value||state.fields.keyKind,state.fields.key);
}
...
function hodlRestoreKey(){
  ...
  [...Zs.children].forEach((button,index)=>{let active=hodlKeyModes[index]===Ne;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))});
  hodlRenderKeyForm();
